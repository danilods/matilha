...
// Wave 5e additions — matilha-sysdesign-pack validation

const SYSDESIGN_PACK_REPO = resolve(__dirname, "../../../matilha-sysdesign-pack");
const sysdesignPackExists = existsSync(SYSDESIGN_PACK_REPO);

const sysdesignPackSkillFrontmatterSchema = z.object({
  name: z.string().regex(/^[a-z][a-z0-9-]*[a-z0-9]$/),
  description: z.string().min(1).max(800),
  category: z.enum(["sysdesign"]),
  version: z.string().regex(/^\d+\.\d+\.\d+$/),
  requires: z.array(z.string()).optional(),
  optional_companions: z.array(z.string()).optional()
});

function loadSysdesignPackSkillFrontmatter(skillDir: string): unknown {
  const content = readFileSync(resolve(SYSDESIGN_PACK_REPO, "skills", skillDir, "SKILL.md"), "utf-8");
  const match = content.match(/^---\n([\s\S]*?)\n---/);
  if (!match) throw new Error(`${skillDir}: SKILL.md has no frontmatter`);
  return parseYaml(match[1]!);
}
...
function loadSysdesignPackSkillContent(skillDir: string): string {
  return readFileSync(resolve(SYSDESIGN_PACK_REPO, "skills", skillDir, "SKILL.md"), "utf-8");
}

function extractSysdesignPackDescriptionWords(skillDir: string): Set<string> {
  const fm = loadSysdesignPackSkillFrontmatter(skillDir) as { description: string };
  const stopwords = new Set(["use", "when", "a", "an", "the", "and", "or", "of", "to", "in", "with", "for", "on", "at", "by"]);
  return new Set(
    fm.description
      .toLowerCase()
...
describe.skipIf(!sysdesignPackExists)("matilha-sysdesign-pack plugin.json (Wave 5e)", () => {
  it("plugin.json declares matilha-pack keyword", () => {
    const raw = readFileSync(resolve(SYSDESIGN_PACK_REPO, ".claude-plugin/plugin.json"), "utf-8");
    const parsed = JSON.parse(raw) as { keywords?: string[] };
    expect(parsed.keywords, "pack plugin.json must include 'matilha-pack' in keywords").toContain("matilha-pack");
  });
  ...
  it("plugin.json has valid top-level shape", () => {
    const raw = readFileSync(resolve(SYSDESIGN_PACK_REPO, ".claude-plugin/plugin.json"), "utf-8");
    const parsed = JSON.parse(raw) as { name?: string; version?: string; license?: string };
    expect(parsed.name).toBe("matilha-sysdesign-pack");
    expect(parsed.version).toMatch(/^\d+\.\d+\.\d+$/);
    expect(parsed.license).toBe("MIT");
  });

  it("marketplace.json parses as valid JSON", () => {
    const raw = readFileSync(resolve(SYSDESIGN_PACK_REPO, ".claude-plugin/marketplace.json"), "utf-8");
    expect(() => JSON.parse(raw)).not.toThrow();
  });
});

describe.skipIf(!sysdesignPackExists)("matilha-sysdesign-pack skill frontmatter (Wave 5e)", () => {
  for (const skillDir of listSysdesignPackSkills()) {
    it(`${skillDir}: frontmatter validates against sysdesignPackSkillFrontmatterSchema`, () => {
      const fm = loadSysdesignPackSkillFrontmatter(skillDir);
      const result = sysdesignPackSkillFrontmatterSchema.safeParse(fm);
      if (!result.success) {
        const issues = result.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join("; ");
        throw new Error(`${skillDir} frontmatter invalid: ${issues}`);
      }
    ...
    it(`${skillDir}: frontmatter name matches directory name`, () => {
      const fm = loadSysdesignPackSkillFrontmatter(skillDir) as { name: string };
      expect(fm.name).toBe(skillDir);
    });
    ...
    it(`${skillDir}: category is sysdesign, matching slug prefix sysdesign-`, () => {
      const fm = loadSysdesignPackSkillFrontmatter(skillDir) as { category: string };
      expect(fm.category).toBe("sysdesign");
      expect(skillDir.startsWith("sysdesign-")).toBe(true);
    });
  }
});

describe.skipIf(!sysdesignPackExists)("matilha-sysdesign-pack skill body (Wave 5e)", () => {
  for (const skillDir of listSysdesignPackSkills()) {
    it(`${skillDir}: body has mandatory ## Sources section`, () => {
      const content = loadSysdesignPackSkillContent(skillDir);
      expect(content, `${skillDir} missing ## Sources section`).toContain("## Sources");
    });
    ...
    it(`${skillDir}: body has all 11 core required sections plus Sources`, () => {
      // Wave 5e accepts "CLI shortcut (optional)" OR "Concrete Example" as the 12th section trailing position.
      // Both are valid pack conventions per SP2a/2b/2c agent choices.
      const content = loadSysdesignPackSkillContent(skillDir);
      const required = [
        "## When this fires",
        "## Preconditions",
...
describe.skipIf(!sysdesignPackExists)("matilha-sysdesign-pack activation uniqueness heuristic (Wave 5e)", () => {
  it("no pair of skills has > 80% description word overlap", () => {
    const skills = listSysdesignPackSkills();
    const wordsBySkill = new Map(skills.map((s) => [s, extractSysdesignPackDescriptionWords(s)]));
    const collisions: string[] = [];
...
describe.skipIf(!sysdesignPackExists)("matilha-sysdesign-pack overlap disclosure (Wave 5e)", () => {
  // Accepts EN or PT forms: "Complements" / "Complementa".
  const overlapSkills = [
    { slug: "sysdesign-nfr-clarification", expectedPhrases: ["Complements", "Complementa"] }
  ];
  for (const { slug, expectedPhrases } of overlapSkills) {
    it(`${slug}: Companion Integration section declares overlap distinction`, () => {
      const content = loadSysdesignPackSkillContent(slug);
      const ciIdx = content.indexOf("## Companion Integration");
      expect(ciIdx).toBeGreaterThan(-1);
      const nextSectionIdx = content.indexOf("\n## ", ciIdx + 1);
      const ciBody = nextSectionIdx > -1 ? content.slice(ciIdx, nextSectionIdx) : content.slice(ciIdx);
...
// Wave 5h additions — matilha-software-arch-pack validation (Caminho C, swarch-*)

const SWARCH_PACK_REPO = resolve(__dirname, "../../../matilha-software-arch-pack");
const swarchPackExists = existsSync(SWARCH_PACK_REPO);

const swarchPackSkillFrontmatterSchema = z.object({
  name: z.string().regex(/^[a-z][a-z0-9-]*[a-z0-9]$/),
  description: z.string().min(1).max(800),
  category: z.enum(["swarch"]),
  version: z.string().regex(/^\d+\.\d+\.\d+$/),
  requires: z.array(z.string()).optional(),
  optional_companions: z.array(z.string()).optional()
});

function loadSwarchPackSkillFrontmatter(skillDir: string): unknown {
  const content = readFileSync(resolve(SWARCH_PACK_REPO, "skills", skillDir, "SKILL.md"), "utf-8");
  const match = content.match(/^---\n([\s\S]*?)\n---/);
  if (!match) throw new Error(`${skillDir}: SKILL.md has no frontmatter`);
  return parseYaml(match[1]!);
}
...
function loadSwarchPackSkillContent(skillDir: string): string {
  return readFileSync(resolve(SWARCH_PACK_REPO, "skills", skillDir, "SKILL.md"), "utf-8");
}

function extractSwarchPackDescriptionWords(skillDir: string): Set<string> {
  const fm = loadSwarchPackSkillFrontmatter(skillDir) as { description: string };
  const stopwords = new Set(["use", "when", "a", "an", "the", "and", "or", "of", "to", "in", "with", "for", "on", "at", "by", "de", "da", "do", "para", "com", "em", "quando", "que", "ao"]);
  return new Set(
    fm.description
      .toLowerCase()
...
describe.skipIf(!swarchPackExists)("matilha-software-arch-pack plugin.json (Wave 5h)", () => {
  it("plugin.json declares matilha-pack keyword", () => {
    const raw = readFileSync(resolve(SWARCH_PACK_REPO, ".claude-plugin/plugin.json"), "utf-8");
    const parsed = JSON.parse(raw) as { keywords?: string[] };
    expect(parsed.keywords, "pack plugin.json must include 'matilha-pack' in keywords").toContain("matilha-pack");
  });
  ...
  it("plugin.json has valid top-level shape", () => {
    const raw = readFileSync(resolve(SWARCH_PACK_REPO, ".claude-plugin/plugin.json"), "utf-8");
    const parsed = JSON.parse(raw) as { name?: string; version?: string; license?: string };
    expect(parsed.name).toBe("matilha-software-arch-pack");
    expect(parsed.version).toMatch(/^\d+\.\d+\.\d+$/);
    expect(parsed.license).toBe("MIT");
  });

  it("marketplace.json parses as valid JSON", () => {
    const raw = readFileSync(resolve(SWARCH_PACK_REPO, ".claude-plugin/marketplace.json"), "utf-8");
    expect(() => JSON.parse(raw)).not.toThrow();
  });
});

describe.skipIf(!swarchPackExists)("matilha-software-arch-pack skill frontmatter (Wave 5h)", () => {
  for (const skillDir of listSwarchPackSkills()) {
    it(`${skillDir}: frontmatter validates against swarchPackSkillFrontmatterSchema`, () => {
      const fm = loadSwarchPackSkillFrontmatter(skillDir);
      const result = swarchPackSkillFrontmatterSchema.safeParse(fm);
      if (!result.success) {
        const issues = result.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join("; ");
        throw new Error(`${skillDir} frontmatter invalid: ${issues}`);
      }
    ...
    it(`${skillDir}: frontmatter name matches directory name`, () => {
      const fm = loadSwarchPackSkillFrontmatter(skillDir) as { name: string };
      expect(fm.name).toBe(skillDir);
    });
    ...
    it(`${skillDir}: category is swarch, matching slug prefix swarch-`, () => {
      const fm = loadSwarchPackSkillFrontmatter(skillDir) as { category: string };
      expect(fm.category).toBe("swarch");
      expect(skillDir.startsWith("swarch-")).toBe(true);
    });
  }
});

describe.skipIf(!swarchPackExists)("matilha-software-arch-pack skill body (Wave 5h)", () => {
  for (const skillDir of listSwarchPackSkills()) {
    it(`${skillDir}: body has mandatory ## Sources section`, () => {
      const content = loadSwarchPackSkillContent(skillDir);
      expect(content, `${skillDir} missing ## Sources section`).toContain("## Sources");
    });
    ...
    it(`${skillDir}: body length is 100-500 lines (Caminho C distillation range)`, () => {
      const content = loadSwarchPackSkillContent(skillDir);
      const lines = content.split("\n").length;
      expect(lines, `${skillDir} body is ${lines} lines; expected 100-500`).toBeGreaterThan(99);
      expect(lines, `${skillDir} body is ${lines} lines; expected 100-500`).toBeLessThan(501);
...
describe.skipIf(!swarchPackExists)("matilha-software-arch-pack activation uniqueness heuristic (Wave 5h)", () => {
  it("no pair of skills has > 80% description word overlap", () => {
    const skills = listSwarchPackSkills();
    const wordsBySkill = new Map(skills.map((s) => [s, extractSwarchPackDescriptionWords(s)]));
    const collisions: string[] = [];
...
describe.skipIf(!swarchPackExists)("matilha-software-arch-pack overlap disclosure (Wave 5h)", () => {
  // 5 skills have Complementa disclosures per Session 1 overlap-analysis.md
  const overlapSkills = [
    { slug: "swarch-dual-store-source-of-truth", expectedPhrases: ["Complements", "Complementa"] },
    { slug: "swarch-event-gateway-boundary", expectedPhrases: ["Complements", "Complementa"] },
    { slug: "swarch-ticker-vs-rule-per-entity", expectedPhrases: ["Complements", "Complementa"] },
    { slug: "swarch-pull-over-push-orchestration", expectedPhrases: ["Complements", "Complementa"] },
    { slug: "swarch-lambda-chain-shape", expectedPhrases: ["Complements", "Complementa"] }
  ];
  for (const { slug, expectedPhrases } of overlapSkills) {
    it(`${slug}: Companion Integration section declares overlap distinction`, () => {
      const content = loadSwarchPackSkillContent(slug);
      const ciIdx = content.indexOf("## Companion Integration");
      expect(ciIdx).toBeGreaterThan(-1);
      const nextSectionIdx = content.indexOf("\n## ", ciIdx + 1);
      const ciBody = nextSectionIdx > -1 ? content.slice(ciIdx, nextSectionIdx) : content.slice(ciIdx);
...
// Wave 5i additions — matilha-security-pack validation (Caminho C, swsec-*, AI-ops security)

const SWSEC_PACK_REPO = resolve(__dirname, "../../../matilha-security-pack");
const swsecPackExists = existsSync(SWSEC_PACK_REPO);

const swsecPackSkillFrontmatterSchema = z.object({
  name: z.string().regex(/^[a-z][a-z0-9-]*[a-z0-9]$/),
  description: z.string().min(1).max(800),
  category: z.enum(["swsec"]),
  version: z.string().regex(/^\d+\.\d+\.\d+$/),
  requires: z.array(z.string()).optional(),
  optional_companions: z.array(z.string()).optional()
});

function loadSwsecPackSkillFrontmatter(skillDir: string): unknown {
  const content = readFileSync(resolve(SWSEC_PACK_REPO, "skills", skillDir, "SKILL.md"), "utf-8");
  const match = content.match(/^---\n([\s\S]*?)\n---/);
  if (!match) throw new Error(`${skillDir}: SKILL.md has no frontmatter`);
  return parseYaml(match[1]!);
}
...
function loadSwsecPackSkillContent(skillDir: string): string {
  return readFileSync(resolve(SWSEC_PACK_REPO, "skills", skillDir, "SKILL.md"), "utf-8");
}

function extractSwsecPackDescriptionWords(skillDir: string): Set<string> {
  const fm = loadSwsecPackSkillFrontmatter(skillDir) as { description: string };
  const stopwords = new Set(["use", "when", "a", "an", "the", "and", "or", "of", "to", "in", "with", "for", "on", "at", "by", "de", "da", "do", "para", "com", "em", "quando", "que", "ao"]);
  return new Set(
    fm.description
      .toLowerCase()
...
describe.skipIf(!swsecPackExists)("matilha-security-pack plugin.json (Wave 5i)", () => {
  it("plugin.json declares matilha-pack keyword", () => {
    const raw = readFileSync(resolve(SWSEC_PACK_REPO, ".claude-plugin/plugin.json"), "utf-8");
    const parsed = JSON.parse(raw) as { keywords?: string[] };
    expect(parsed.keywords, "pack plugin.json must include 'matilha-pack' in keywords").toContain("matilha-pack");
  });
  ...
  it("plugin.json has valid top-level shape", () => {
    const raw = readFileSync(resolve(SWSEC_PACK_REPO, ".claude-plugin/plugin.json"), "utf-8");
    const parsed = JSON.parse(raw) as { name?: string; version?: string; license?: string };
    expect(parsed.name).toBe("matilha-security-pack");
    expect(parsed.version).toMatch(/^\d+\.\d+\.\d+$/);
    expect(parsed.license).toBe("MIT");
  });

  it("marketplace.json parses as valid JSON", () => {
    const raw = readFileSync(resolve(SWSEC_PACK_REPO, ".claude-plugin/marketplace.json"), "utf-8");
    expect(() => JSON.parse(raw)).not.toThrow();
  });
});

describe.skipIf(!swsecPackExists)("matilha-security-pack skill frontmatter (Wave 5i)", () => {
  for (const skillDir of listSwsecPackSkills()) {
    it(`${skillDir}: frontmatter validates against swsecPackSkillFrontmatterSchema`, () => {
      const fm = loadSwsecPackSkillFrontmatter(skillDir);
      const result = swsecPackSkillFrontmatterSchema.safeParse(fm);
      if (!result.success) {
        const issues = result.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join("; ");
        throw new Error(`${skillDir} frontmatter invalid: ${issues}`);
      }
    ...
    it(`${skillDir}: frontmatter name matches directory name`, () => {
      const fm = loadSwsecPackSkillFrontmatter(skillDir) as { name: string };
      expect(fm.name).toBe(skillDir);
    });
    ...
    it(`${skillDir}: category is swsec, matching slug prefix swsec-`, () => {
      const fm = loadSwsecPackSkillFrontmatter(skillDir) as { category: string };
      expect(fm.category).toBe("swsec");
      expect(skillDir.startsWith("swsec-")).toBe(true);
    });
  }
});

describe.skipIf(!swsecPackExists)("matilha-security-pack skill body (Wave 5i)", () => {
  for (const skillDir of listSwsecPackSkills()) {
    it(`${skillDir}: body has mandatory ## Sources section`, () => {
      const content = loadSwsecPackSkillContent(skillDir);
      expect(content, `${skillDir} missing ## Sources section`).toContain("## Sources");
    });
...
describe.skipIf(!swsecPackExists)("matilha-security-pack activation uniqueness heuristic (Wave 5i)", () => {
  it("no pair of skills has > 80% description word overlap", () => {
    const skills = listSwsecPackSkills();
    const wordsBySkill = new Map(skills.map((s) => [s, extractSwsecPackDescriptionWords(s)]));
    const collisions: string[] = [];
...
describe.skipIf(!swsecPackExists)("matilha-security-pack overlap disclosure (Wave 5i)", () => {
  const overlapSkills = [
    { slug: "swsec-rate-limiting-as-defense", expectedPhrases: ["Complements", "Complementa"] },
    { slug: "swsec-llm-cost-as-availability", expectedPhrases: ["Complements", "Complementa"] }
  ];
  for (const { slug, expectedPhrases } of overlapSkills) {
    it(`${slug}: Companion Integration section declares overlap distinction`, () => {
      const content = loadSwsecPackSkillContent(slug);
      const ciIdx = content.indexOf("## Companion Integration");
      expect(ciIdx).toBeGreaterThan(-1);
      const nextSectionIdx = content.indexOf("\n## ", ciIdx + 1);
      const ciBody = nextSectionIdx > -1 ? content.slice(ciIdx, nextSectionIdx) : content.slice(ciIdx);
