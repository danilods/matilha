...
\`matilha-compose\` classifies intent into a matilha phase (discover / spec / design / split / merge / review / status), detects installed companion packs, and dispatches to the right skill with pack-aware enrichment.

If \`matilha:matilha-compose\` is NOT visible in your skill list, matilha is not installed — proceed with default skills.

**Phases:** 0 (status) → 10 (discover) → 20-30 (spec/approve) → 40 (split/merge) → 50 (review) → 60 (den) → 70 (pack).
