...
export async function howlCommand(cwd: string, opts: HowlOptions): Promise<void> {
  ...
  lines.push(`  ${"current".padEnd(LABEL_WIDTH)}phase ${s.current_phase} ${c.dim(`(${s.phase_status})`)}`);
  ...
  if (s.current_phase >= 10 && s.current_phase <= 30) {
    ...
    lines.push(`  ${"gates remaining".padEnd(LABEL_WIDTH)}${total - done} of ${total}`);
  }

  lines.push(`  ${"next action".padEnd(LABEL_WIDTH)}${s.next_action}`);
  lines.push("");

  // Activity section
  lines.push(c.bold("Activity"));
  lines.push(`  ${"tools".padEnd(LABEL_WIDTH)}${s.tools_detected.join(", ") || c.dim("(none)")}`);
  lines.push(`  ${"active waves".padEnd(LABEL_WIDTH)}${s.active_waves.length > 0 ? s.active_waves.join(", ") : c.dim("(none)")}`);

  if (s.pending_decisions.length > 0) {
    lines.push(`  ${"pending decisions".padEnd(LABEL_WIDTH)}${c.yellow(String(s.pending_decisions.length))}`);
    for (const d of s.pending_decisions) lines.push(c.yellow(`    • ${d}`));
  } else {
    lines.push(`  ${"pending decisions".padEnd(LABEL_WIDTH)}${c.dim("(none)")}`);
  }

  if (s.blockers.length > 0) {
    lines.push(`  ${"blockers".padEnd(LABEL_WIDTH)}${c.red(String(s.blockers.length))}`);
    for (const b of s.blockers) lines.push(c.red(`    • ${b}`));
  } else {
    lines.push(`  ${"blockers".padEnd(LABEL_WIDTH)}${c.dim("(none)")}`);
  }
