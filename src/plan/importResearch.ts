...
export function importResearch(filePath: string): ImportedResearch {
  if (!existsSync(filePath)) {
    throw new MatilhaUserError({
      ...
      context: "matilha spec --import-research expects a deep-research markdown file",
  ...
  if (!/\.(md|markdown)$/i.test(filePath)) {
    throw new MatilhaUserError({
      ...
      context: "matilha spec --import-research only accepts markdown (.md or .markdown)",
      ...
      example: "matilha spec user-auth --import-research ./research.md"
    });
  }
  ...
  if (size > MAX_SIZE_BYTES) {
    throw new MatilhaUserError({
      ...
      context: "matilha spec embeds the research file inline into the spec's Section 1",
  ...
  if (content.trim().length === 0) {
    throw new MatilhaUserError({
      ...
      context: "matilha spec --import-research needs actual content to embed as Section 1",
